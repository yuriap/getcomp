...
Rem    pedany      11/06/17 - version 2.0
Rem                           HTML report format is only supported from this time
Rem    pedany      04/11/24 - version 3.0
Rem                           Adapted for SQLCL as a single script file to use "ora_mining_varchar2_nt" Oracle table type to produce output by a table function 
Rem
...
spool getcomp_&output_file_suff..html

set define @

with 
  function get_report return ora_mining_varchar2_nt
  is
  l_report ora_mining_varchar2_nt := ora_mining_varchar2_nt();
  ...
  l_css clob:=
q'[body.awr {font:bold 10pt Arial,Helvetica,Geneva,sans-serif;color:black; background:White;}
...
--l_rn          number := 0;
--begin 
--l_data :=  rtrim(p_msg,chr(10))||chr(10);
--loop
--  l_row := substr(l_data,1,instr(l_data,chr(10))-1);
--  l_data := substr(l_data,instr(l_data,chr(10))+1);
--  exit when l_row is null;
--  l_report.extend;
--  l_report(l_report.count) := dbms_xplan_type(substr(l_row,1,300));  
--  l_rn:=l_rn+1;
--  exit when l_rn>1000;
--end loop;
--end;
procedure p(p_msg varchar2, p_delim varchar2 default ';') 
is 
l_data varchar2(32765);
l_row varchar2(32767);
l_val varchar2(32767);
...
l_vn          number;
l_pos         number;
l_row_len constant number:=4000;
begin 
l_data :=  rtrim(p_msg,chr(10))||chr(10);
loop
  l_row := substr(l_data,1,instr(l_data,chr(10))-1);
  l_data := substr(l_data,instr(l_data,chr(10))+1);
  exit when l_row is null;
  if length(l_row) > l_row_len then
    if instr(l_row,p_delim)>0 then
      l_vn := 0;
      l_row := rtrim(l_row,p_delim)||p_delim;
      loop
        l_pos := nvl(instr(substr(l_row,1,l_row_len),p_delim,-1),l_row_len);
        l_val :=  substr(l_row,1,l_pos);
        exit when l_val is null;
        l_row := substr(l_row,l_pos+1);
        l_report.extend;
        l_report(l_report.count) := l_val; --dbms_xplan_type(l_val);   
        l_vn := l_vn + 1;
        exit when l_vn > 1000;
      end loop; 
    end if;
      ...
      l_report.extend;
      l_report(l_report.count) := substr(l_row,1,l_row_len);--dbms_xplan_type(substr(l_row,1,l_row_len));  
  end if;
  l_rn:=l_rn+1;
  exit when l_rn>1000;
end loop;
...
procedure print_table_html(p_query in varchar2,
  ...
  l_widest number := 0;
  l_indx number := 1;
  procedure pin(p_line varchar2) is
  begin
    l_output(l_indx):=p_line;
    l_indx := l_indx + 1;
  end;  
  procedure p1(p_msg varchar2) is begin p(p_msg); end;
  procedure output is
  begin
...
begin
  pin(HTF.TABLEOPEN(cborder=>0,cattributes=>'width="<width>" class="tdiff" summary="'||p_summary||'"'));
  ...
  --column names
  pin(HTF.TABLEROWOPEN);
  for i in 1 .. l_colCnt loop
    pin(HTF.TABLEHEADER(cvalue=>l_descTbl(i).col_name,calign=>'left',cattributes=>'class="awrbg" scope="col"'));
  end loop;
  pin(HTF.TABLEROWCLOSE);
    ...
    for i in 1 .. l_colCnt loop
      ...
      if p_search is not null then
        if instr(l_descTbl(i).col_name,p_search)>0 then
          ...
          pin(HTF.TABLEDATA(cvalue=>l_columnValue,calign=>'left',cattributes=>'class="'|| l_style ||'"'));
        elsif regexp_instr(l_columnValue,p_search)>0 then
          l_columnValue:=REGEXP_REPLACE(l_columnValue,p_search,p_replacement);
          pin(HTF.TABLEDATA(cvalue=>l_columnValue,calign=>'left',cattributes=>'class="'|| l_style ||'"'));
        else
          pin(HTF.TABLEDATA(cvalue=>replace(l_columnValue,'  ','&nbsp;&nbsp;'),calign=>'left',cattributes=>'class="'|| l_style ||'"'));
        end if;
      else
        pin(HTF.TABLEDATA(cvalue=>replace(l_columnValue,'  ','&nbsp;&nbsp;'),calign=>'left',cattributes=>'class="'|| l_style ||'"'));
      end if;
...
begin
  ...
  pin(HTF.TABLEOPEN(cborder=>0,cattributes=>'width="<width>" class="tdiff" summary="'||p_t_header||'"'));
  if p_t_header<>'#FIRST_LINE#' then
    pin(HTF.TABLEROWOPEN);
    pin(HTF.TABLEHEADER(cvalue=>replace(p_t_header,' ','&nbsp;'),calign=>'left',cattributes=>'class="awrbg" scope="col"'));
    pin(HTF.TABLEROWCLOSE);
  end if;
  ...
  loop
    ...
    if p_t_header='#FIRST_LINE#' and l_iter = 1 then
      pin(HTF.TABLEROWOPEN);
      pin(HTF.TABLEHEADER(cvalue=>replace(l_line,' ','&nbsp;'),calign=>'left',cattributes=>'class="awrbg" scope="col"'));
      pin(HTF.TABLEROWCLOSE);
    else
      pin(HTF.TABLEROWOPEN);
      ...
      if p_comparison and l_pref in ('-') then
        pin(HTF.TABLEDATA(cvalue=>l_line,calign=>'left',cattributes=>'class="'|| case when mod(l_iter,2)=0 then l_style_comp1 else l_style_comp2 end ||'"'));
      else
        pin(HTF.TABLEDATA(cvalue=>l_line,calign=>'left',cattributes=>'class="'|| case when mod(l_iter,2)=0 then l_style1 else l_style2 end ||'"'));
      end if;
